...
const afterStartCh = dc.channel('dd-trace:confluent-kafka-javascript:consumer:afterStart')
const beforeFinishCh = dc.channel('dd-trace:confluent-kafka-javascript:consumer:beforeFinish')

class ConfluentKafkaJavacriptConsumerPlugin extends ConsumerPlugin {
  static get id () { return 'confluent-kafka-javascript' }
  static get operation () { return 'consume' }

  constructor () {
    super(...arguments)
    this.addSub('apm:confluent-kafka-javascript:consume:commit', message => this.commit(message))
  }

  /**
   * Transform individual commit details sent by confluent-kafka-javascript' event reporter
   ...
   * @property {string} topic
   * @property {number} partition
   * @property {import('confluent-kafka-javascript/utils/long').Long} offset
   *
...
module.exports = ConfluentKafkaJavacriptConsumerPlugin
