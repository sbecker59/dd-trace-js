...
describe('Plugin', () => {
  describe('confluent-kafka-javascript', function () {
    this.timeout(10000) // TODO: remove when new internal trace has landed
    afterEach(() => {
      return agent.close({ ritmReset: false })
    })
    withVersions('confluent-kafka-javascript', 'confluent-kafka-javascript', (version) => {
      let kafka
      let tracer
      let Kafka
      describe('without configuration', () => {
        ...
        beforeEach(async () => {
          ...
          await agent.load('confluent-kafka-javascript')
          const lib = require(`../../../versions/confluent-kafka-javascript@${version}`).get()
          Kafka = lib.Kafka
          kafka = new Kafka({
            clientId: `confluent-kafka-javascript-test-${version}`,
        ...
        describe('producer', () => {
          it('should be instrumented', async () => {
            const expectedSpanPromise = expectSpanWithDefaults({
              ...
              meta: {
                'span.kind': 'producer',
                component: 'confluent-kafka-javascript',
          ...
          // Dynamic broker list support added in 1.14/2.0 (https://github.com/tulios/confluent-kafka-javascript/commit/62223)
        ...
        describe('consumer', () => {
          ...
          it('should publish on afterStart channel', (done) => {
            const afterStart = dc.channel('dd-trace:confluent-kafka-javascript:consumer:afterStart')
          ...
          it('should publish on beforeFinish channel', (done) => {
            const beforeFinish = dc.channel('dd-trace:confluent-kafka-javascript:consumer:beforeFinish')
        ...
        describe('data stream monitoring', () => {
          ...
          beforeEach(async () => {
            tracer.init()
            tracer.use('confluent-kafka-javascript', { dsmEnabled: true })
            consumer = kafka.consumer({ groupId: 'test-group' })
            await consumer.connect()
            await consumer.subscribe({ topic: testTopic })
          })
